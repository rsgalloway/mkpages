...
def generate_site(
    ...
    effective_site_url = (
        parse_site_url(Path("command line"), site_url) if site_url is not None else site_config.url
    )
    effective_site_baseurl = (
        parse_baseurl(Path("command line"), site_baseurl)
        if site_baseurl is not None
        else site_config.baseurl
    )
    ...
    warnings = write_site_files(
        ...
        site_baseurl=effective_site_baseurl,
...
def write_site_files(
    ...
    write_config(output_dir, site_title, site_description, site_url, site_baseurl)
...
def write_config(
    ...
    config = render_template(
        ...
        site_url_line=f"url: {json.dumps(site_url)}\n" if site_url else "",
        site_baseurl_line=f"baseurl: {json.dumps(site_baseurl)}\n" if site_baseurl else "",
    )
...
def write_layouts(
    ...
    (layout_dir / "default.html").write_text(
        render_template(
            ...
            social_card_markup=build_social_card_markup(resolved_card, site_url),
        ),
        encoding="utf-8",
    )
...
def build_social_card_markup(resolved_card: ResolvedCard, site_url: str | None) -> str:
    """Render Open Graph and Twitter card metadata."""
    if not resolved_card.enabled:
        return ""

    url_filter = "absolute_url" if site_url else "relative_url"
    image_href = "{{ '/" + resolved_card.image_path.as_posix() + "' | " + url_filter + " }}"
...
def parse_site_config(raw_text: str, config_path: Path) -> SiteConfig:
    ...
    return SiteConfig(
        ...
        url=parse_site_url(config_path, scalars.get("url")),
        baseurl=parse_baseurl(config_path, scalars.get("baseurl")),
...
def parse_site_url(config_path: Path, raw_value: str | None) -> str | None:
    """Validate a configured canonical site URL."""
    if raw_value is None:
        return None

    value = raw_value.strip().rstrip("/")
    if not value:
        raise MkpagesError(f"url in {config_path} must not be empty")
    if not (value.startswith("http://") or value.startswith("https://")):
        raise MkpagesError(f"url in {config_path} must start with http:// or https://")
    if any(ch in value for ch in {'"', "'", "<", ">", "\\", "\n", "\r", "\t"}):
        raise MkpagesError(
            f"url in {config_path} must not contain quotes, angle brackets, backslashes, or control characters"
        )
    return value


def parse_baseurl(config_path: Path, raw_value: str | None) -> str | None:
    """Validate an optional Jekyll baseurl."""
    if raw_value is None:
        return None
    ...
    if not value.startswith("/"):
        raise MkpagesError(f"baseurl in {config_path} must start with '/' when set")
    if value.endswith("/"):
        value = value.rstrip("/")
    if any(ch in value for ch in {'"', "'", "<", ">", "\\", "\n", "\r", "\t", " "}):
        raise MkpagesError(
            f"baseurl in {config_path} must not contain spaces, quotes, angle brackets, backslashes, or control characters"
        )
    return value
